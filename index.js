...
function addGamesToPage(games) {
    ...
    // loop over each item in the data
        for(let i=0; i<games.length; i++){
            ...
            // about each game
            //include image and at least 2 other features
            gameCard.innerHTML = `
...
const unfundedgames = GAMES_JSON.reduce((acc, games)=> acc + (games.pledged < games.goal? 1:0),0);
//raisedCard.innerHTML = unfundedgames.toLocaleString();
// create a string that explains the number of unfunded games using the ternary operator

const str = unfundedgames==1? 'game remains':'games remain';

// create a new DOM element containing the template string and append it to the description container
const displaystr = `A totoal of $${TotalRaised.toLocaleString()} has been raised for ${GAMES_JSON.length} games. 
                    Currently, ${unfundedgames} ${str} unfunded. We need your help to fund these amazing games! `
